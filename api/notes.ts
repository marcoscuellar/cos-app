...
//
// Private per-user: the user id comes from the session cookie only; keyed
// `user:<uid>:notes:<project>`. No project's notes are ever reachable cross-user.
// ─────────────────────────────────────────────────────────────────────────────
...
const keyFor = (uid: string, project: string): string => `user:${uid}:notes:${project}`;

async function readNotes(uid: string, project: string): Promise<Note[]> {
  const notes = await kvGet<Note[]>(keyFor(uid, project));
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === "GET") {
      ...
      return res.status(200).json({ notes: await readNotes(uid, project) });
    }
    ...
    if (req.method === "POST") {
      ...
      const notes = [note, ...(await readNotes(uid, project))].slice(0, MAX_NOTES);
      await kvSet(keyFor(uid, project), notes);
    ...
    if (req.method === "DELETE") {
      ...
      const notes = (await readNotes(uid, project)).filter((n) => n.id !== id);
      await kvSet(keyFor(uid, project), notes);
