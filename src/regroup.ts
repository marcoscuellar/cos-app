...
// The live plan Regroup reasons over — built from the signed-in user's real rooms
// (projects) and today's DayPlan (intention + to-dos). See src/regroupData.ts.
export interface RegroupPlan {
  oneThing: { title: string; roomId: string | null; roomName: string | null };
  todos: { id: string; title: string; essential?: boolean }[];
  thread: { id: string; name: string; pick: string; when: string }[];
}
...
// contract as Ask Ollin / Brainstorm.
export function computeMove(mode: RegroupMode, plan: RegroupPlan): RegroupMove {
  switch (mode) {
    case "cantstart": {
